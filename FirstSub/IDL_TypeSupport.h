...
public:
	...
	static void print_data(UserDataType* pUserDataType,int layer = 0)
	{
		...
		std::cout<<c<<"  send_counter = "<< pUserDataType->send_counter <<std::endl;
		std::cout<<c<<"  md5_str = "<< pUserDataType->md5_str <<std::endl;
		std::cout<<c<<"  time = "<< pUserDataType->time <<std::endl;
		std::cout<<c<<"  file_content = "<< pUserDataType->file_content <<std::endl;
	}
